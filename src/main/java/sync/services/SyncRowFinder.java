...
public class SyncRowFinder {
    ...
    public SyncRowFinder(AdapterDao adapterDao, Adapter adapter) {
        ...
        try {
            ...
            for(int i = 1; i < masterTable.getRows().size(); i++){
                ...
                for(int j = 1; j < slaveTable.getRows().size(); j++){
                    ...
                    if(adapter.convertRow(masterTable.getRows().get(i), masterTable.getRows().get(0)).getValues()
                            .equals(slaveTable.getRows().get(j).getValues())){
